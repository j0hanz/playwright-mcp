...
<body>
  ...
  <div class="violation ${escapeHtml(v.impact || 'minor')}">
    ...
    // (Agent Integration - Planner, Generator, Healer)
    // ==========================================
    ...
    this.server.registerTool(
      'test_plan_create',
      {
        ...
        description:
          'Create a Markdown test plan specification for Playwright Test Generator agent',
        inputSchema: {
          name: z
            .string()
            .describe(
              'Test plan name (e.g., login-flow, checkout-process). Will be saved as specs/{name}.md'
            ),
          content: z.string().describe('Markdown test plan content'),
        },
        outputSchema: {
          success: z.boolean(),
          path: z.string(),
      ...
      this.createToolHandler(async ({ name, content }) => {
        const { promises: fs } = await import('fs');
    ...
    this.server.registerTool(
      ...
      this.createToolHandler(async ({ path: filePath, content, reason }) => {
        const { promises: fs } = await import('fs');

        await fs.writeFile(filePath, content, 'utf-8');

        this.logger.info('Test file updated', {
          path: filePath,
    ...
    this.server.registerTool(
      ...
      this.createToolHandler(async ({ path: filePath }) => {
        const { promises: fs } = await import('fs');
    ...
    this.server.registerTool(
      ...
      this.createToolHandler(async ({ type }) => {
        const { promises: fs } = await import('fs');
        ...
        try {
          ...
          const err = toError(error);
          this.logger.error('Error listing test artifacts', {
            error: err.message,
          });
        }
    ...
    this.server.registerTool(
      ...
      this.createToolHandler(async ({ path: filePath }) => {
        ...
        try {
          ...
          this.logger.info('Test artifact deleted', { path: filePath });
  ...
  private registerResources(): void {
    ...
    // ==========================================
    // Agent Artifacts Resources
    // (Test Plans and Test Specifications)
    // ==========================================

    // Test Plans Resource List
    this.server.registerResource(
      'test-plans-list',
      'playwright://artifacts/specs',
      {
        title: 'Test Plans Directory',
        description:
          'Directory of all Markdown test plan specifications (used by Playwright Test Generator)',
        mimeType: 'application/json',
      },
      // sync implementation but SDK expects async callback
      () => {
        try {
          // Using dynamic require is necessary here for sync resource callback
          const { readdirSync } = require('fs') as typeof import('fs');
          const specsFiles: Array<{ isFile: () => boolean; name: string }> =
            readdirSync('specs', { withFileTypes: true });
          const specs = specsFiles
            .filter(
            ...
            .map((file: { name: string }) => ({
              name: file.name,
              path: `specs/${file.name}`,
              uri: `playwright://artifacts/specs/${file.name}`,
            }));

          return {
            contents: [
              {
                uri: 'playwright://artifacts/specs',
                mimeType: 'application/json',
                text: JSON.stringify({
                  type: 'test-plans',
                  count: specs.length,
          ...
          this.logger.error('Failed to list test plans', {
            error: toError(error).message,
          });
          return {
            contents: [
    ...
    // Test Specifications Resource List
    this.server.registerResource(
      'test-specs-list',
      'playwright://artifacts/tests',
      {
        title: 'Test Specifications Directory',
        description:
          'Directory of all generated Playwright test specification files',
        mimeType: 'application/json',
      },
      // sync implementation but SDK expects async callback
      () => {
        try {
          // Using dynamic require is necessary here for sync resource callback
          const { readdirSync } = require('fs') as typeof import('fs');
          const testsFiles: Array<{ isFile: () => boolean; name: string }> =
            readdirSync('tests', { withFileTypes: true });
          const tests = testsFiles
            .filter(
              (file: { isFile: () => boolean; name: string }) =>
                file.isFile() && file.name.endsWith('.spec.ts')
            )
            .map((file: { name: string }) => ({
              name: file.name,
              path: `tests/${file.name}`,
              uri: `playwright://artifacts/tests/${file.name}`,
            }));

          return {
            contents: [
              {
                uri: 'playwright://artifacts/tests',
                mimeType: 'application/json',
                text: JSON.stringify({
          ...
          this.logger.error('Failed to list test specs', {
            error: toError(error).message,
          });
          return {
            contents: [
